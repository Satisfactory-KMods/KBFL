			...
			if (bIsCheatEnabled && bIsHostEnableCheatMode)
			{
				if (mPlayerController)
				{
					...
					if (Character)
					{
						...
						{
							UFGCharacterMovementComponent* CharacterMovement = Cast<UFGCharacterMovementComponent>(
				...
				if (bIsNoPower != mCheatManager->NoCost_Get() || bIsNoPower != mCheatManager->NoPower_Get() ||
					bIsNoPower != mCheatManager->NoFuel_Get())
				{
					mCheatManager->NoCost(bIsNoPower);
					mCheatManager->NoFuel(bIsNoPower);
					mCheatManager->NoPower(bIsNoPower);
					UE_LOG(LogTemp, Warning,
					       TEXT("Update Cheats NoCost_Get( %d ), NoPower_Get( %d ), NoFuel_Get( %d )"),
					       mCheatManager->NoCost_Get(), mCheatManager->NoPower_Get(), mCheatManager->NoFuel_Get())
				}
			}
...
void AKBFLUtilSubsystem::UnlockAll()
{
	if (HasAuthority())
	{
		if (mCheatManager)
		{
			...
			const UModContentRegistry* ContentRegistry = UModContentRegistry::Get(GetWorld());
