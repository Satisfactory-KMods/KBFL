...
void UKBFLResourceNodeDescriptor_ResourceNode::AfterSpawning() {
	...
	if(mAddToOilPump && mResourceClass && Sub) {
		if(TSubclassOf<AFGBuildableResourceExtractor> BPOilPump = LoadClass<AFGBuildableResourceExtractor>(nullptr, TEXT("/Game/FactoryGame/Buildable/Factory/OilPump/Build_OilPump.Build_OilPump_C"))) {
