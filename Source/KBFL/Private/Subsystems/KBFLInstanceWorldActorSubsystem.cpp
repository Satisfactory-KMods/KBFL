...
bool AKBFLInstanceWorldActorSubsystem::KBFL_OverwriteInstanceData_Transform(
	AFGBuildable* Buildable, UStaticMesh* Mesh, FTransform NewRelativTransform, int32 Idx)
{
	if (!IsValid(Buildable))
	{
		return false;
	...
	int32                                                                   InstanceIdx, bool MarkDirty)
{
	...
	if (Buildable->mInstanceHandles.IsValidIndex(InstanceIdx))
	{
		if (Buildable->mInstanceHandles[InstanceIdx]->IsInstanced())
		{
			...
			Buildable->mInstanceHandles[InstanceIdx]->SetPrimitiveDataByID(
				Data/** float that we want to set */, FloatIndex /** Index where we want to set */, true);
			return true;
		}
	}
...
bool AKBFLInstanceWorldActorSubsystem::KBFL_SetInstanceHidden(AFGBuildable* Buildable, int32 InstanceIdx, bool IsHidden)
{
	...
	if (Buildable->mInstanceHandles.IsValidIndex(InstanceIdx))
	{
		if (Buildable->mInstanceHandles[InstanceIdx]->IsInstanced())
		{
			if (UHierarchicalInstancedStaticMeshComponent* Hism = Buildable->mInstanceHandles[InstanceIdx]->
				GetInstanceComponent())
			{
				const int32 HandleID = Buildable->mInstanceHandles[InstanceIdx]->GetHandleID();

				FTransform T = Buildable->mInstanceDataCDO->GetInstanceData()[InstanceIdx].RelativeTransform * Buildable
					->GetActorTransform();
				T.SetScale3D(!IsHidden ? T.GetScale3D() : FVector(1.e-4f));
				Buildable->mInstanceHandles[InstanceIdx]->SetLocalTransform(T);
			}
			return true;
		}
	}
	...
	FTransform                                                                      Transform)
{
	...
	if (Buildable->mInstanceHandles.IsValidIndex(InstanceIdx))
	{
		if (Buildable->mInstanceHandles[InstanceIdx]->IsInstanced())
		{
			Buildable->mInstanceHandles[InstanceIdx]->SetLocalTransform(Transform);
			return true;
		}
	}
