			...
			if (Swatch.mSwatch)
			{
				...
				if (SwatchDefauls)
				{
					...
					if (!mSwatchIDMap.Find(ColourIndex))
					{
						if (mDefaultSwatchCollection)
						{
							UFGFactoryCustomizationCollection* Default = CDOHelperSubsystem->
								GetAndStoreDefaultObject_Native<UFGFactoryCustomizationCollection>(
						...
						if (!Subsystem->mColorSlots_Data.IsValidIndex(ColourIndex))
						{
							...
							for (uint8 i = Subsystem->mColorSlots_Data.Num(); i <= ColourIndex; ++i)
							{
								...
								FFactoryCustomizationColorSlot NewColourSlot = FFactoryCustomizationColorSlot(
									Swatch.mPrimaryColour, Swatch.mSecondaryColour);
						...
						if (!FGGameState->mBuildingColorSlots_Data.IsValidIndex(ColourIndex))
						{
							...
							FGGameState->mBuildingColorSlots_Data[ColourIndex] = Subsystem->mColorSlots_Data[
								ColourIndex];
							FGGameState->SetupColorSlots_Data(Subsystem->mColorSlots_Data);
		...
		for (FKBFLMaterialDescriptorInformation Information : CDOInformation)
		{
			if (Information.mApplyThisInformationTo.Num() > 0)
			{
				for (auto Descriptor : Information.mApplyThisInformationTo)
				{
					if (Descriptor)
					{
						if (UFGFactoryCustomizationDescriptor_Material* Default = CDOHelperSubsystem->
							GetAndStoreDefaultObject_Native<UFGFactoryCustomizationDescriptor_Material>(Descriptor))
						{
...
void UKBFLCustomizerSubsystem::BeginForModule(UWorldModule* Module)
{
	if (Module && !WasForModuleCalled(Module))
	{
		...
		{
			const FTimerDelegate RespawnDelegate = FTimerDelegate::CreateUObject(
				this, &UKBFLCustomizerSubsystem::BeginForModule, Module);
