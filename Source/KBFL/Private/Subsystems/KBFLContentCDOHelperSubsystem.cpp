...
void UKBFLContentCDOHelperSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	TimerHandle,                    
	this,                              
	&UKBFLContentCDOHelperSubsystem::OnTimerCallback,  
	5.0f,                              
	false                               
);
...
void UKBFLContentCDOHelperSubsystem::OnTimerCallback()
{
	...
	UE_LOG(ContentCDOHelperSubsystem, Error, TEXT("UKBFLContentCDOHelperSubsystem::OnTimerCallback %d"), mCategoriesToCall.Num());
}
                                                             ...
                                                             TArray<TSubclassOf<UFGRecipe>>                                           IgnoreRecipe)
{
	...
	if (!SubFrom) UE_LOG(ContentCDOHelperSubsystem, Error, TEXT("CDO_MoveRecipesFromBuilding -> Cannot load From"));

	if (!SubTo) UE_LOG(ContentCDOHelperSubsystem, Error, TEXT("CDO_MoveRecipesFromBuilding -> Cannot load To"));
	...
	if (SubTo && SubFrom)
	{
		...
		for (auto Recipe : Recipes)
		{
			if (IsValid(Recipe) && !IgnoreRecipe.Contains(Recipe))
			{
				...
				if (IsValid(Default))
				{
					if (!IgnoreCategory.Contains(Default->mOverriddenCategory) || Default->mOverriddenCategory ==
						nullptr)
					{
						Default->mProducedIn.Remove(From);
						Default->mProducedIn.Add(To);
						UE_LOG(ContentCDOHelperSubsystem, Error, TEXT("CDO_MoveRecipesFromBuilding -> Move Recipe %s : %s -> %s"),
	...
	bool                                                                                  IgnoreCallCheck)
{
	...
	{
		if (UKBFL_CDOHelperClass_Base* CDOHelper = GetAndStoreDefaultObject_Native<UKBFL_CDOHelperClass_Base>(
			CDOHelperClass))
		{
