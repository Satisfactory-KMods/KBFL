...
void UKBFL_CDOHelperClass_CategoryOrder::DoCDO()
{
	...
	{
		UFGBuildCategory* DefaultCategoryObject = mSubsystem->GetAndStoreDefaultObject_Native<UFGBuildCategory>(TargetCategory);
		DefaultCategoryObject->mMenuPriority = mTargetCategoryPrio;
	}

	if (mSubTargetCategoryPrio > -1000.f)
	{
		UFGBuildSubCategory* DefaultSubCategoryObject = mSubsystem->GetAndStoreDefaultObject_Native<UFGBuildSubCategory>(TargetSubCategory);
