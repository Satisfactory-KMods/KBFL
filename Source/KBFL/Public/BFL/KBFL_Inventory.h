...
public:
	...
	static void FindFirstAllowedSlot(UFGInventoryComponent* Inventory, TSubclassOf<UFGItemDescriptor> Item,
		KBFLFoundSomething&                                 Found, int32&                             Index);

	/** Return all Index where the ItemClass allowed on Slot in Inventory */
	UFUNCTION(BlueprintCallable, Category="KMods|Items")
	static void FindAllAllowedSlots(UFGInventoryComponent* Inventory, TSubclassOf<UFGItemDescriptor> Item,
